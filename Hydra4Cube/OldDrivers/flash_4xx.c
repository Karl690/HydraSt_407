...
HYREL_FLASH_Status HYREL_FLASH_EraseSector(uint32_t FLASH_Sector, uint8_t VoltageRange)
{
  ...
  HYREL_FLASH_Status status = HYREL_FLASH_COMPLETE;

  /* Check the parameters */
  assert_param(IS_FLASH_SECTOR(FLASH_Sector));
  assert_param(IS_VOLTAGERANGE(VoltageRange));
  
  if(VoltageRange == HYREL_VoltageRange_1)
  {
     tmp_psize = HYREL_FLASH_PSIZE_BYTE;
  }
  else if(VoltageRange == HYREL_VoltageRange_2)
  {
    tmp_psize = HYREL_FLASH_PSIZE_HALF_WORD;
  }
  else if(VoltageRange == HYREL_VoltageRange_3)
  {
    tmp_psize = HYREL_FLASH_PSIZE_WORD;
  }
  else
  {
    tmp_psize = HYREL_FLASH_PSIZE_DOUBLE_WORD;
  }
  /* Wait for last operation to be completed */
  status = HYREL_FLASH_WaitForLastOperation();
  
  if(status == HYREL_FLASH_COMPLETE)
  { 
...
HYREL_FLASH_Status HYREL_FLASH_ProgramDoubleWord(uint32_t Address, uint64_t Data)
{
  ...
  /* Check the parameters */
  assert_param(HYREL_IS_FLASH_ADDRESS(Address));
...
/**
  ...
  *                       FLASH_ERROR_WRP, FLASH_ERROR_OPERATION or FLASH_COMPLETE.
  */
HYREL_FLASH_Status FLASH_ProgramWord(uint32_t Address, uint32_t Data)
{
	HYREL_FLASH_Status status = HYREL_FLASH_COMPLETE;
...
/**
  ...
  */
uint16_t HYREL_FLASH_OB_GetWRP(void)
{
...
/**
  ...
  */
FlagStatus HYREL_FLASH_OB_GetRDP(void)
{
  FlagStatus readstatus = RESET;

  if ((*(__IO uint8_t*)(HYREL_OPTCR_BYTE1_ADDRESS) != (uint8_t)HYREL_OB_RDP_Level_0))
  {
    readstatus = SET;
  }
  else
...
/**
  ...
  */
uint8_t HYREL_FLASH_OB_GetBOR(void)
{
...
{
	HYREL_FLASH_Status flashstatus = HYREL_FLASH_COMPLETE;
  
  if((FLASH->SR & FLASH_FLAG_BSY) == HYREL_FLASH_FLAG_BSY)
  {
  ...
  {  
    ...
    { 
      flashstatus = HYREL_FLASH_ERROR_WRP;
    }
    else 
    {
      if((FLASH->SR & (uint32_t)0xEF) != (uint32_t)0x00)
      {
        flashstatus = HYREL_FLASH_ERROR_PROGRAM;
      }
      else
      {
        if((FLASH->SR & FLASH_FLAG_OPERR) != (uint32_t)0x00)
        {
          flashstatus = HYREL_FLASH_ERROR_OPERATION;
        }
        else
        {
          flashstatus = HYREL_FLASH_COMPLETE;
        }
      }
    }
  }
...
{ 
  __IO HYREL_FLASH_Status status = HYREL_FLASH_COMPLETE;
   
  /* Check for the FLASH Status */
  status = HYREL_FLASH_GetStatus();
