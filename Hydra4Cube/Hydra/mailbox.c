...
{
	HYREL_FLASH_Status status;

	if (_sysInfoPtr->soapSize != 0)
	{
		HYREL_FLASH_Unlock();
		status = HYREL_FLASH_EraseSector(_sysInfoPtr->soapSector, HYREL_VoltageRange_3);
		HYREL_FLASH_Lock();
...
void writeSystemSoapString(int section)
{
	...
	{   // do write - assumes erase has already occurred
		...
		{
			HYREL_FLASH_ProgramByte((uint32_t)soapWritePtr, *newDataPtr);
			soapWritePtr++;
			if (*newDataPtr == '\0')
			{
				break;
...
void eraseSystemCrashLog(void)
{
	...
	{   // only if there is a crashlog block in flash
		...
		HYREL_FLASH_Unlock();
		status = HYREL_FLASH_EraseSector(_sysInfoPtr->crashlogSector, HYREL_VoltageRange_3);
		HYREL_FLASH_Lock();
		if (status != HYREL_FLASH_COMPLETE)
		{
...
void sendCrashDataFromRAMtoFlash(void)
{
	...
	{
		HYREL_FLASH_Unlock();
		if (HYREL_FLASH_EraseSector(FLASH_1024_CRASH_SECTOR, HYREL_VoltageRange_3) != HYREL_FLASH_COMPLETE)  { return; } //  can't do anything
		HYREL_FLASH_Lock();
	...
	{
		HYREL_FLASH_Unlock();
		if (HYREL_FLASH_EraseSector(FLASH_2048_CRASH_SECTOR, HYREL_VoltageRange_3) != HYREL_FLASH_COMPLETE)  { return; } //  can't do anything
		HYREL_FLASH_Lock();
