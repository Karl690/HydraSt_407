...
sliceTimingStruct _sliceTiming;
int _motionTimerCalls = 0;

void initSliceTiming(boolean initNames)
{
	sliceStruct *p;
	int i;

	_sliceTiming.matchSlice = -1;
	_sliceTiming.counterPeriod = 0.000001; // 1 usec
#ifdef GB_SLICE_TIMING_PIN
	_sliceTiming.pin = GB_SLICE_TIMING_PIN;
#else
	_sliceTiming.pin = PIN_UNDEFINED;
#endif
	InitTimer2usec();
	...
	for (i=0; i<NUM_SLICE_TIME_ENTRIES; i++)
	{
		...
		//rcpy(p->name, "123456789 12345");  // 15 chars + NULL
		if (initNames)
			strcpy(p->name, "UNDEFINED__NAME");  // 15 chars + NULL
		p->minTime = MAXINT;
		p->maxTime = MININT;
		p->motionTimerCalls = MININT;
	}
	if (initNames)
	{
		int these_are_out_of_date;
		//rcpy(_sliceTiming.slices[0].name, "123456789 12345");
		strcpy(_sliceTiming.slices[0].name, "ohNoMrBill     ");
		strcpy(_sliceTiming.slices[1].name, "CommandProcessr");
		strcpy(_sliceTiming.slices[2].name, "SequenceEngine ");
		strcpy(_sliceTiming.slices[3].name, "canPrepNextRx  ");
		strcpy(_sliceTiming.slices[4].name, "canProcessRxQue");
		strcpy(_sliceTiming.slices[5].name, "canProcessTxQue");
		strcpy(_sliceTiming.slices[6].name, "motionQ_update ");
		strcpy(_sliceTiming.slices[7].name, "DwellTimer     ");
		strcpy(_sliceTiming.slices[8].name, "spare          ");
		strcpy(_sliceTiming.slices[9].name, "spare          ");

		//rcpy(_sliceTiming.slices[10].name, "123456789 12345");
		strcpy(_sliceTiming.slices[10].name, "ohNoMrBill     ");
		strcpy(_sliceTiming.slices[11].name, "dacup          ");
		strcpy(_sliceTiming.slices[12].name, "readInputs     ");
		strcpy(_sliceTiming.slices[13].name, "initFromSoapStr");
		strcpy(_sliceTiming.slices[14].name, "spare          ");
		strcpy(_sliceTiming.slices[15].name, "spare          ");
		strcpy(_sliceTiming.slices[16].name, "spare          ");
		strcpy(_sliceTiming.slices[17].name, "spare          ");
		strcpy(_sliceTiming.slices[18].name, "spare          ");
		strcpy(_sliceTiming.slices[19].name, "loop_100Hz_work");

		//rcpy(_sliceTiming.slices[10].name, "123456789 12345");
		strcpy(_sliceTiming.slices[20].name, "ohNoMrBill     ");
		strcpy(_sliceTiming.slices[21].name, "A2DRead10      ");
		strcpy(_sliceTiming.slices[22].name, "A2DRead11      ");
		strcpy(_sliceTiming.slices[23].name, "printLine1     ");
		strcpy(_sliceTiming.slices[24].name, "soapstringContr");
		strcpy(_sliceTiming.slices[25].name, "sendUpdateToHos");
		strcpy(_sliceTiming.slices[26].name, "spare          ");
		strcpy(_sliceTiming.slices[27].name, "spare          ");
		strcpy(_sliceTiming.slices[28].name, "spare          ");
		strcpy(_sliceTiming.slices[29].name, "loop_10Hz_work ");

		//rcpy(_sliceTiming.slices[10].name, "123456789 12345");
		strcpy(_sliceTiming.slices[30].name, "systicker      ");
		strcpy(_sliceTiming.slices[31].name, "UpdateDisplay  ");
		strcpy(_sliceTiming.slices[32].name, "SmoothData     ");
		strcpy(_sliceTiming.slices[33].name, "checkForMia    ");
		strcpy(_sliceTiming.slices[34].name, "reportCalTemp  ");
		strcpy(_sliceTiming.slices[35].name, "ReportXYZLocati");
		strcpy(_sliceTiming.slices[36].name, "spare          ");
		strcpy(_sliceTiming.slices[37].name, "spare          ");
		strcpy(_sliceTiming.slices[38].name, "spare          ");
		strcpy(_sliceTiming.slices[39].name, "loop_1Hz_work  ");
	}
}
#endif
...
motionQStruct motionQ  __attribute__ ((section (".ccmram")));
GMCommandStructure cmdQue[SIZE_OF_COMMAND_QUEUE]  __attribute__ ((section (".ccmram")));
GMCommandStructure *ExecutionPtr  __attribute__ ((section (".ccmram")));
GMCommandStructure *NextExecutionPtr  __attribute__ ((section (".ccmram")));   //NextExecutionPtr=&cmdQue[NextCommandIndex]; tmp pointer
int NextCommandIndex  __attribute__ ((section (".ccmram")));         // index of the next command to be executed (the one after Current), initially set to 2 after the first command is received and parsed
int CurrentCommandIndex  __attribute__ ((section (".ccmram")));      // index of the current place in cmdQue[] from which to get prams for execution
int NextCommandInsertionIndex  __attribute__ ((section (".ccmram")));// index of the next free location to add an incomming command
char *GCodeArgPtr  __attribute__ ((section (".ccmram")));
//unsigned int counter_1Hz  __attribute__ ((section (".ccmram")));
//unsigned int counter_10Hz  __attribute__ ((section (".ccmram")));
//unsigned int counter_100Hz  __attribute__ ((section (".ccmram")));
//unsigned int counter_1000Hz  __attribute__ ((section (".ccmram")));
#else
...
#ifdef USE_HYREL_IO
	initHss(X_HSS1_INDEX, X_HSS1);
	initHss(X_HSS2_INDEX, X_HSS2);
	initHss(Y_HSS1_INDEX, Y_HSS1);
	initHss(Y_HSS2_INDEX, Y_HSS2);
	initHss(Z_HSS1_INDEX, Z_HSS1);
	initHss(Z_HSS2_INDEX, Z_HSS2);
	initHss(W_HSS1_INDEX, W_HSS1);
	initHss(W_HSS2_INDEX, W_HSS2);
#elif defined (USE_HYDRA_IO)
...
void checkMotorFaultSensor(void)
{
	_motionSensorTrippedNext = checkMotorSensor(SENSOR_INDEX_FAULT);
}
void checkMotorLimit1Sensor(void)
{
	_motionSensorTrippedNext |= checkMotorSensor(SENSOR_INDEX_LIMIT1);
}
void checkMotorLimit2Sensor(void)
{
...
#ifdef MEASURE_ISR_RATE
void measureIsrRate(void)
{   // see TIM2_ISR .... will toggle a pin each update of TIM2 ... use LA to determine ISR call RATE.
	TIM_TimeBaseInitTypeDef MyTIM_TimeBaseInitStruct;

	initClkAndResetAPB1(RCC_APB1Periph_TIM2);

	MyTIM_TimeBaseInitStruct.TIM_Prescaler = 0;
	MyTIM_TimeBaseInitStruct.TIM_CounterMode = TIM_CounterMode_Up;
	MyTIM_TimeBaseInitStruct.TIM_Period = 11; //20; // 0 PSC + 11 Period --> update every 0.25usec. . (84=1usec, 42=0.5, 21=0.25, 12=0.14usec
	...
	interruptSetupAndEnable(TIM2_IRQn, NVIC_PREMPTION_PRIORITY_MEASURE_ISR_RATE);
...
{
#ifdef SLICE_TIMING_MEASUREMENT
	initSliceTiming(TRUE);
#endif

#ifdef MEASURE_ISR_RATE
#warning "MEASURE_ISR_RATE enabled - use LA to count ISR rate (program cannot do motion)"
	measureIsrRate(); // this is blocking and only used for debug/manual testing
#endif // MEASURE_ISR_RATE

#ifdef GB_DEBUG_LSI_FPU
	sprintf(_tmpStr, "LSI=%d", (int)lsiActualFreq);  sendGB(_tmpStr);
#if defined(__FPU_PRESENT) && defined(__FPU_USED)
	sprintf(_tmpStr, "__FPU_PRESENT && __FPU_USED");  sendGB(_tmpStr);
#elif defined(__FPU_PRESENT)
	sprintf(_tmpStr, "__FPU_PRESENT");  sendGB(_tmpStr);
#elif defined(__FPU_USED)
	sprintf(_tmpStr, "__FPU_USED");  sendGB(_tmpStr);
#else
	sprintf(_tmpStr, "NO FPU");  sendGB(_tmpStr);
#endif
#endif //GB_DEBUG_LSI_FPU

#ifdef GB_DEBUGGING_KEEP_HEADS_ALIVE
	InvalidateAllCmdArgs(ExecutionPtr);
	ARG_K = 1.0f;   // ignore comm watchdogs -- keep heads alive
	M_Code_M719();
#endif //GB_DEBUGGING

#ifdef MEASURE_TIME_SLIPPAGE
	InitTimer2ForTimeSlipMeasurement(); // start just prior to enabling systick timer
#endif //MEASURE_TIME_SLIPPAGE

#ifdef GB_STARTUP_PIN
	pinSet(GB_STARTUP_PIN);
...
#ifdef ALLOW_NATIVE_LIGHTBURN
	while (_bootCounterMs < BOOT_COUNT_DONE_MS)
	{
		_heartbeatRateControl = HEARTBEAT_MODE_NORMAL;
	} // _lightburnModeEnabled will be the var to test
#endif //ALLOW_NATIVE_LIGHTBURN
...
//#warning "*******************************************************************"
//#warning "remove master4.c"
//#warning "*******************************************************************"
#ifdef GB_DEBUGGING
#warning "GB_DEBUGGING is ENABLED -- non production code"
#endif //GB_DEBUGGING
#ifdef USE_PANEL_ABSEL_IO_FOR_DEBUG_PINS
#warning "AB encoder unavailable - pins have been stolen for debug"
#endif //USE_PANEL_ABSEL_IO_FOR_DEBUG_PINS
#ifdef USE_PANEL_6TO1_IO_FOR_DEBUG_PINS
#warning "6:1 select unavailable - pins have been stolen for debug"
#endif //USE_PANEL_6TO1_IO_FOR_DEBUG_PINS
...
#warning "TIM2 used for debug: measuring time -- local control on CO2 Laser unavailable"
